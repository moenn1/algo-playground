...
- Added a shared graph replay for Redundant Connection, including deterministic input-order edge inspection, explicit accepted-forest ledgers, and replay-safe first-cycle reporting through the Union-Find runtime.
- Wired Redundant Connection through the input-service presets and validation, replay shell, library catalog, seeded demo data, and graph-family documentation so TraceDeck now covers a distinct first-cycle detection problem alongside traversal, weighted broadcast, Union-Find tree validation, scheduling, infection, component counting, dual-ocean reachability, island bridging, blocked-grid pathfinding, capture, and room-filling replays.
- Expanded the seeded demo dataset and operator docs so local verification now reflects 37 algorithms with runs, 74 total runs, and 2 saved comparisons.
...
- Added a shared graph replay for Pacific Atlantic Water Flow, including deterministic dual-ocean border seeding, uphill reverse-flow reachability, immediate dual-ocean intersection tracking, and replay-safe coastline state without hidden flood-fill reconstruction.
- Wired Pacific Atlantic Water Flow through the input-service presets and validation, replay shell, library catalog, seeded demo data, and graph-family documentation so TraceDeck now covers a distinct dual-ocean reachability problem alongside traversal, Union-Find, scheduling, infection, component counting, capture, and room-filling replays.
- Expanded the seeded demo dataset and operator docs so local verification now reflects 33 algorithms with runs, 66 total runs, and 2 saved comparisons.
...
- Updated the operator runbook and seeded-dataset totals so local replay, history, and persistence checks now reflect all 29 currently supported algorithms with 58 seeded runs.

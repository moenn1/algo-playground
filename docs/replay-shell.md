...
- The graph stage now runs on the shared execution-engine package for Breadth-First Search, Depth-First Search, Dijkstra, Graph Valid Tree, Course Schedule, Rotting Oranges, Number of Islands, and Walls and Gates, so queue order, top-first stack order, weighted frontier order, Union-Find root comparison, zero-indegree unlocks, minute-wave infection spread, row-major scan checkpoints, room-distance fills, and terminal stall reporting all come from one deterministic runtime family.
- Graph replay now pairs the SVG graph map or grid stage with a structural-state rail for node, course, or cell status, distance, representative-root, parent-rank, indegree, or room-distance inspection, plus path-, accepted-edge-, order-, infection-, connectivity-, or distance-fill focus so the interface can surface graph state without inventing browser-only metadata.

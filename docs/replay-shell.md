...
- Sorting replay now ships through a reusable stage module that adds an operation summary, live trace metrics, and a per-lane ledger so the same component can serve Bubble Sort, Insertion Sort, Selection Sort, Quick Sort, Merge Sort, Heap Sort, the main interface, and future page-level layouts.
...
- The graph stage now runs on the shared execution-engine package for Breadth-First Search, Dijkstra, Course Schedule, Rotting Oranges, Number of Islands, and Walls and Gates, so queue order, weighted frontier order, zero-indegree unlocks, minute-wave infection spread, row-major scan checkpoints, room-distance fills, and terminal stall reporting all come from one deterministic runtime family.
- Graph replay now pairs the SVG graph map or grid stage with a structural-state rail for node, course, or cell status, distance or indegree inspection, and path-, order-, infection-, connectivity-, or distance-fill focus so the interface can surface graph state without inventing browser-only metadata.

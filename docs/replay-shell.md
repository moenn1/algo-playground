...
- The reference library exposes a catalog route, per-algorithm detail routes, and named-problem detail routes with pattern-group organization, interview framing, related problems, implementation variants, and four language solutions that can open the live replay surface when an associated algorithm exists.
...
- As the study catalog grows, named problems should be treated as first-class routes organized by pattern family instead of being folded into generic algorithm blurbs.
- Keep expanding well-known interview problems even when some pages are study-first references without replay linkage yet; those pages should still ship full pattern context and real solution code.

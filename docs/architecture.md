...
- Graph engines currently share `settled`, `frontier`, `inspections`, and `updates` so BFS, DFS, Dijkstra, Clone Graph, Graph Valid Tree, Course Schedule, Rotting Oranges, Number of Islands, Pacific Atlantic Water Flow, Shortest Path in Binary Matrix, Surrounded Regions, and Walls and Gates expose one stable graph-runtime vocabulary to replay and persistence consumers, even when the settled work unit is a processed edge ledger, a copied-node ledger, a dual-ocean flow source, a traceback-ready search cell, or a captured-cell ledger instead of a queue-driven node expansion.

...
- Publishes stable graph semantics for frontier ordering, settled nodes, inspections, updates, zero-indegree unlocks, minute checkpoints, row-major scan checkpoints, room-distance fills, and terminal cycle or stall reporting across the graph family
...
- Graph engines currently share `settled`, `frontier`, `inspections`, and `updates` so BFS, Dijkstra, Course Schedule, Rotting Oranges, Number of Islands, and Walls and Gates expose one stable graph-runtime vocabulary to replay and persistence consumers.

...
- Shares a replay-safe graph state union across Breadth-First Search, Depth-First Search, Dijkstra, Network Delay Time, Clone Graph, Graph Valid Tree, Count Connected Components, Redundant Connection, Course Schedule, Course Schedule II, Rotting Oranges, Number of Islands, Max Area of Island, Island Perimeter, Pacific Atlantic Water Flow, Shortest Bridge, Shortest Path in Binary Matrix, Nearest Exit from Entrance in Maze, Shortest Path in a Grid with Obstacles Elimination, Minimum Obstacle Removal to Reach Corner, Swim in Rising Water, Shortest Path to Get Food, 01 Matrix, As Far from Land as Possible, Map of Highest Peak, Surrounded Regions, and Walls and Gates so the UI and persistence layers can render pathfinding, weighted broadcast, clone construction, Union-Find validation, connected-component counting, first-cycle detection, dependency scheduling, returned course-order replay, grid BFS infection replay, largest-island area ledgers, coastline-perimeter ledgers, dual-ocean reverse-flow replay, first-island marking plus bridge-wave expansion, blocked-cell shortest-path traceback, nearest-boundary-exit traceback, obstacle-budget dominance pruning, minimum-removal deque replay, weighted water-level traceback, pantry-target traceback, nearest-zero distance matrices, shoreline-distance ledgers, water-seeded height maps, border-safe capture replay, and room-distance filling without browser-only reconstruction
...
- Publishes stable graph semantics for queue, stack, and weighted-frontier ordering, settled nodes, inspections, updates, zero-indegree unlocks, minute checkpoints, row-major scan checkpoints, dual-ocean ledgers, blocked-cell ledgers, traceback paths, border-safe ledgers, capture queues, room-distance fills, and terminal cycle or stall reporting across the graph family
...
- Graph engines currently share `settled`, `frontier`, `inspections`, and `updates` so BFS, DFS, Dijkstra, Network Delay Time, Clone Graph, Graph Valid Tree, Count Connected Components, Redundant Connection, Course Schedule, Course Schedule II, Rotting Oranges, Number of Islands, Max Area of Island, Island Perimeter, Pacific Atlantic Water Flow, Shortest Path in Binary Matrix, Nearest Exit from Entrance in Maze, Shortest Path in a Grid with Obstacles Elimination, Minimum Obstacle Removal to Reach Corner, Shortest Path to Get Food, 01 Matrix, As Far from Land as Possible, Map of Highest Peak, Surrounded Regions, and Walls and Gates expose one stable graph-runtime vocabulary to replay and persistence consumers, even when the settled work unit is a processed edge ledger, a weighted broadcast relay, a copied-node ledger, a component-count checkpoint, a first-cycle verdict edge, a returned topological-order checkpoint, a largest-island scoreboard update, a coastline-perimeter checkpoint, a dual-ocean flow source, a traceback-ready search cell, a nearest-boundary-exit checkpoint, an obstacle-budget state checkpoint, a minimum-removal deque checkpoint, a pantry-target checkpoint, a nearest-zero distance checkpoint, a shoreline-distance checkpoint, a height-assignment checkpoint, or a captured-cell ledger instead of a queue-driven node expansion.

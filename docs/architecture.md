...
- Shares a replay-safe graph state union across Breadth-First Search, Dijkstra, Course Schedule, Rotting Oranges, and Number of Islands so the UI and persistence layers can render pathfinding, dependency scheduling, grid BFS infection replay, and connected-component counting without browser-only reconstruction
...
- Publishes stable graph semantics for frontier ordering, settled nodes, inspections, updates, zero-indegree unlocks, minute checkpoints, row-major scan checkpoints, and terminal cycle or stall reporting across the graph family
...
- Graph engines currently share `settled`, `frontier`, `inspections`, and `updates` so BFS, Dijkstra, Course Schedule, Rotting Oranges, and Number of Islands expose one stable graph-runtime vocabulary to replay and persistence consumers.

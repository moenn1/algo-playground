...
export interface NearestExitFromEntranceInMazeInputPayload extends JsonObject {
  grid: string[][];
  entrance: [number, number];
}

export interface ShortestPathGridWithObstaclesEliminationInputPayload extends JsonObject {
  grid: number[][];
  eliminations: number;
}

export interface MinimumObstacleRemovalToReachCornerInputPayload extends JsonObject {
  grid: number[][];
}
...
export interface MapOfHighestPeakInputPayload extends JsonObject {
  grid: number[][];
}
